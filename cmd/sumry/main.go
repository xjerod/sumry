...
func init() {
	flag.StringVar(&projectRoot, "project-root", ".", "the project root sumry will parse")
...
func main() {
	...
	log.Debug("checking archive dir", "dir", sumryArchiveDir)
	if _, err := os.Stat(sumryArchiveDir); err != nil {
		if os.IsNotExist(err) {
			log.Warn("archive dir doesn't exist, creating it")
			if merr := os.Mkdir(sumryArchiveDir, 0o755); merr != nil {
				log.Fatal("failed to create archive dir", "err", err)
			}
		} else {
			log.Fatal("failed to check archive dir", "err", err)
		}
	}
